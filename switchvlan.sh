...
#cat /etc/network/interfaces` — контроль, что VLAN-подинтерфейсы прописаны правильно.
#sysctl net.ipv4.ip_forward` — проверка, что маршрутизация включена (`= 1`).
#grep -n "net.ipv4.ip_forward" /etc/sysctl.conf` — подтверждение строки в конфиге.
...
#iptables -t nat -L -n -v` — проверка NAT: должно быть правило `MASQUERADE` на выход в интернет (`ens3`).
#iptables -L FORWARD -n -v` — проверка разрешения форвардинга между офисом (`ens4`) и интернетом (`ens3`).
#iptables-save | head` — быстрая проверка, что правила вообще сохраняются/есть.
#ping -c 2 8.8.8.8` — проверка выхода в интернет по IP с роутера.
#ping -c 2 google.com` — проверка DNS-резолвинга на роутере (если нужно).
...
#ip -br a` — проверка, что `vlan999` поднят и имеет IP `192.168.99.2/24`.
#ip r` — проверка маршрута по умолчанию свитча через `192.168.99.1` (router в VLAN 999).
#ping -c 2 192.168.99.1` — проверка связи управления: свитч ↔ роутер по VLAN 999.
#ping -c 2 8.8.8.8` — проверка выхода свитча в интернет через роутер (NAT).
#ping -c 2 google.com` — проверка DNS на свитче.
#apt update` — проверка, что пакеты скачиваются (интернет точно работает).

## SERVER (VLAN 100)

#ip -br a` — проверка IP на `ens3`: должен быть `192.168.100.10/24`.
#ip r` — проверка default gateway: должен быть `192.168.100.1`.
...
#ping -c 2 8.8.8.8` — проверка интернета по IP.
#ping -c 2 google.com` — проверка DNS.
#apt update` — контроль, что сервер тоже имеет доступ к репозиториям.

## CLIENT (VLAN 200)

#ip -br a` — проверка IP на `ens3`: должен быть `192.168.200.10/24`.
#ip r` — проверка default gateway: должен быть `192.168.200.1`.
...
#ping -c 2 8.8.8.8` — проверка интернета по IP.
#ping -c 2 google.com` — проверка DNS.
#apt update` — контроль доступа к репозиториям.
